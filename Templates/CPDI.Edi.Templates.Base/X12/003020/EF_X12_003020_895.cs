namespace EdiFabric.Templates.X12003020
{
    using System;
    using System.Collections.Generic;
    using System.Runtime.Serialization;
    using System.Xml.Serialization;
    using EdiFabric.Core.Annotations.Edi;
    using EdiFabric.Core.Annotations.Validation;
    using EdiFabric.Core.Model.Edi;
    using EdiFabric.Core.Model.Edi.X12;


    /// <summary>
    /// Loop for Line Item Detail Adjustment
    /// </summary>
    [Serializable()]
    [DataContract()]
    [Group(typeof(G89))]
    public class Loop_G89_895
    {

        [XmlIgnore]
        [IgnoreDataMember]
        public int Id { get; set; }
        /// <summary>
        /// Line Item Detail Adjustment
        /// </summary>
        [DataMember]
        [Required]
        [Pos(1)]
        public virtual G89 G89 { get; set; }
        /// <summary>
        /// Pricing Information
        /// </summary>
        [DataMember]
        [Pos(2)]
        public virtual G22 G22 { get; set; }
        /// <summary>
        /// Allowance or Charge
        /// </summary>
        [DataMember]
        [ListCount(10)]
        [Pos(3)]
        public virtual List<G72> G72 { get; set; }
        /// <summary>
        /// Terms of Sale
        /// </summary>
        [DataMember]
        [ListCount(20)]
        [Pos(4)]
        public virtual List<G23> G23 { get; set; }
    }

    /// <summary>
    /// Loop for Loop Header
    /// </summary>
    [Serializable()]
    [DataContract()]
    [Group(typeof(LS))]
    public class Loop_LS_895
    {

        [XmlIgnore]
        [IgnoreDataMember]
        public int Id { get; set; }
        /// <summary>
        /// Loop Header
        /// </summary>
        [DataMember]
        [Required]
        [Pos(1)]
        public virtual LS LS { get; set; }
        /// <summary>
        /// Loop for Line Item Detail Adjustment
        /// </summary>
        [DataMember]
        [ListCount(9999)]
        [Pos(2)]
        public virtual List<Loop_G89_895> G89Loop { get; set; }
        /// <summary>
        /// Loop Trailer
        /// </summary>
        [DataMember]
        [Required]
        [Pos(3)]
        public virtual LE LE { get; set; }
    }

    /// <summary>
    /// Delivery/Return Acknowledgement and/or Adjustment
    /// </summary>
    [Serializable()]
    [DataContract()]
    [Message("X12", "895")]
    public class TS895 : EdiMessage
    {

        [XmlIgnore]
        [IgnoreDataMember]
        public int Id { get; set; }
        /// <summary>
        /// Transaction Set Header
        /// </summary>
        [DataMember]
        [Pos(1)]
        public virtual ST ST { get; set; }
        /// <summary>
        /// Delivery/Return Adjustment Identification
        /// </summary>
        [DataMember]
        [Required]
        [Pos(2)]
        public virtual G87 G87 { get; set; }
        /// <summary>
        /// Delivery/Return Identification Adjustment
        /// </summary>
        [DataMember]
        [Pos(3)]
        public virtual G88 G88 { get; set; }
        /// <summary>
        /// Loop for Loop Header
        /// </summary>
        [DataMember]
        [Pos(4)]
        public virtual Loop_LS_895 LSLoop { get; set; }
        /// <summary>
        /// Allowance or Charge
        /// </summary>
        [DataMember]
        [ListCount(20)]
        [Pos(5)]
        public virtual List<G72> G72 { get; set; }
        /// <summary>
        /// Terms of Sale
        /// </summary>
        [DataMember]
        [ListCount(20)]
        [Pos(6)]
        public virtual List<G23> G23 { get; set; }
        /// <summary>
        /// Delivery/Return Record of Totals
        /// </summary>
        [DataMember]
        [Pos(7)]
        public virtual G84 G84 { get; set; }
        /// <summary>
        /// Signature
        /// </summary>
        [DataMember]
        [Required]
        [Pos(8)]
        public virtual G86 G86 { get; set; }
        /// <summary>
        /// Record Integrity Check
        /// </summary>
        [DataMember]
        [Required]
        [Pos(9)]
        public virtual G85 G85 { get; set; }
        /// <summary>
        /// Transaction Set Trailer
        /// </summary>
        [DataMember]
        [Pos(10)]
        public virtual SE SE { get; set; }
    }
}
